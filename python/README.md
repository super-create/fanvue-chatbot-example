# Fanvue Chatbot - Python Example

A simple chatbot application that integrates with the Fanvue API using OAuth 2.0 authentication.

## Prerequisites

Before you begin, make sure you have:

1. **Python 3.8 or later** installed on your computer

   - Check by running: `python3 --version` or `python --version`
   - Download from: https://www.python.org/downloads/

2. **A Fanvue account** with API access

   - Request API access at: https://help.fanvue.com/en/articles/11363222-request-api-keys

3. **An OAuth application** created in the Fanvue Developer area
...
3. Fill in the application details:
   ...
   - **Redirect URI**: `https://your-app-name.dev:5001/callback` (replace `your-app-name` with your chosen name)
...
OAUTH_REDIRECT_URI=https://your-app-name.dev:5001/callback
...
4. Update your OAuth application Redirect URI in the Fanvue Developer area to match: `https://your-app-name.dev:5001/callback`
...
In your terminal (in the `python` directory), run:
...
Open a **new terminal window**, navigate to the `python` directory, and run (replace `your-app-name` with your chosen app name):

```bash
npx local-ssl-proxy --source 5001 --target 5000 --cert ./your-app-name.dev.pem --key ./your-app-name.dev-key.pem
```

You should see output like:

```
Proxying https://your-app-name.dev:5001 -> http://localhost:5000
```
...
2. Go to: `https://your-app-name.dev:5001` (replace `your-app-name` with your chosen app name)
...
- Try: `pip install -r requirements.txt` instead

### "Invalid redirect URI" error

- Make sure the Redirect URI in your Fanvue OAuth app settings exactly matches: `https://your-app-name.dev:5001/callback` (replace `your-app-name` with your chosen app name)
- Check your `.env` file has the correct `OAUTH_REDIRECT_URI` with HTTPS
- Verify the domain matches exactly (including the `.dev` extension and port `5001`)
...
- Another application might be using port 5000 or 5001
- For port 5000: You can change the port in `app.py` (last line: `app.run(debug=True, port=5000)`)
- For port 5001: Change the `--source` parameter in the `local-ssl-proxy` command
...
- Make sure you're accessing the app via `https://your-app-name.dev:5001` (not `http://`)
...
- Make sure both terminal windows are running (Flask app and SSL proxy)
- Try accessing `https://127.0.0.1:5001` instead

### Proxy connection refused

- Ensure the Flask app is running on port 5000 before starting the SSL proxy
...
1. Customize the chatbot's responses in `app.py`
